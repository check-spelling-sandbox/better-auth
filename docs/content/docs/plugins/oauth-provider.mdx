...
Accept or deny user consent for a set of scopes. Note that when denying scopes, the consent cancels and preexisting consent remains. To remove consent, delete that user's "oauthConsent" for that client.
