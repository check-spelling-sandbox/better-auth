...
No falling back to database. Best for high-performance scenarios where all keys are migrated to secondary storage.
...
Check secondary storage first, then fall back to database if not found.
