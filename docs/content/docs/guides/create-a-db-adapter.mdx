...
<Callout type="info">
  In v1.4, adapter tests used `runAdapterTest` and `runNumberIdAdapterTest` from `better-auth/adapters/test`. That export was removed in v1.5. Use `@better-auth/test-utils/adapter` instead.
</Callout>
